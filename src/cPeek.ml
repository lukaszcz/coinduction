...
let declare_peek_fun evd ind_name =
  ...
  let branches =
    List.map2
      begin fun (lc, i) p ->
        let lc = EConstr.of_constr lc in
        let lc = CNorm.norm_beta evd (mkApp(mkLambda (Name.Anonymous, tyind, lc), [| mkInd ind |])) in
      ...
      (List.combine (Array.to_list one_ind.mind_nf_lc) (Array.to_list one_ind.mind_consnrealargs))
      (range 1 (Array.length one_ind.mind_nf_lc + 1))
  in
  let ci = Inductiveops.make_case_info (Global.env ()) ind MatchStyle in
  ...
  let ret = close mkLambda typeargs (mkLambda (Name.Anonymous, ty, ty0)) in
  let ret = CNorm.norm_beta evd (mkApp (ret, Array.of_list params2)) in
  let case = mkCase(ci, ret, mkRel 1, Array.of_list branches) in
  let peek = close mkLambda typeargs (mkLambda (Name.Anonymous, ty, case)) in
...
let declare_peek_eq evd ind_name =
  ...
  let ty0 =
    mkApp (get_constr "eq",
           ...
           |])
  in
  let ret0 = close mkLambda typeargs (mkLambda (Name.Anonymous, ty, ty0)) in
  let ret = CNorm.norm_beta evd (mkApp (ret0, Array.of_list params2)) in
  let case = mkCase(ci, ret, mkRel 1, Array.of_list branches) in
  let peek_eq = close mkLambda typeargs (mkLambda (Name.Anonymous, ty, case)) in
